...
if($and == 1){
  if(isset($_REQUEST['nom_plan'])){
    $Planturistico = $_REQUEST['nom_plan'];
    if ($Planturistico != "" ){
      if($where == ""){
      $where="WHERE nom_plan = '$Planturistico'";
    }
  }
}
}

  if(isset($_REQUEST['dest_plan'])){
    $destino = $_REQUEST['dest_plan'];
    if ($destino != "" ){
      if($where == ""){
          $where="WHERE dest_plan = '$destino'";
      }

      else {
        $where = "$where AND dest_plan = '$destino'";
      }

    }
  }
  if(isset($_REQUEST['nom_age'])){
    $nombreagencia = $_REQUEST['nom_age'];
    if ($nombreagencia != "" ){
      if($where == ""){
          $where="WHERE ag.nom_age = '$nombreagencia'";
      }

      else {
        $where = "$where AND ag.nom_age = '$nombreagencia'";
      }
...
$sql = "SELECT cod_plan,nom_plan,dest_plan,valor_plan,nom_age FROM planturistico as pl JOIN agencias as ag ON pl.nit_age = ag.nit_age $where";
